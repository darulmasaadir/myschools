...
`Employee` rows are branch-scoped by `api/permissions.employee_query` (Phase 8d).

### Payroll Entry (upstream `hrms.Payroll Entry`)

Phase 8d — present only when `frappe/hrms` is installed (it's in `required_apps`).

| Field | Type | Target | Notes |
|---|---|---|---|
| `mys_branch` | Link | MYS Branch | scopes the run; `Company` aligned to the branch's books by `api/hr.set_payroll_entry_company_from_branch` |
| `mys_campus` | Link | MYS Campus | |

Branch-scoped by `api/hr.payroll_entry_query`.
