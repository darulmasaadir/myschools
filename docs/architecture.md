...
| Fees | 🟡 | upstream `education.Fees` + `Fee Structure`; Phase **8a**: `MYS Fee Structure Override`, `MYS Late Fee Policy`, [`api/fees.py`](../frappe-bench/apps/myschools/myschools/api/fees.py) | Submitted Fees → royalty via `get_branch_collection_for_period`. Per-branch/campus fee structure overrides + daily late-fee job (8a in flight). |
