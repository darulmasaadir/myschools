...
| Communication / Payments | 🟡 | `MYS Communication Log` (+ `Payment` channel); SMS adapters (**8c** ✅ PR #18); payment adapters (**8e** 🟡): `MYS Payment Settings`, `initiate_fee_payment` | Royalty + inspection alerts; pluggable SMS + Pakistani fee gateways (JazzCash / Easypaisa / HBL). [process](processes/notifications.md) |
