...
| Communication | 🟡 | `MYS Communication Log`; 6 `Email Template` + 6 `Notification` fixtures; `Communication.after_insert` mirror; `send_sms` + `send_email_message` adapters (Phase **8c** in flight) | Royalty + inspection + agreement alerts wired; system emails mirrored into the audit log. [process](processes/notifications.md) |
